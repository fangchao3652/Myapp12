...
import com.opensky.BuildConfig;
...
public class DiskLruImageCache implements ImageCache {
    ...
    public DiskLruImageCache(String filePath, int diskCacheSize, CompressFormat compressFormat, int quality) {
        try {
            final File diskCacheDir = new File(filePath);
            mDiskCache = DiskLruCache.open(diskCacheDir, APP_VERSION, VALUE_COUNT, diskCacheSize);
            mCompressFormat = compressFormat;
            mCompressQuality = quality;
        } catch (IOException e) {
